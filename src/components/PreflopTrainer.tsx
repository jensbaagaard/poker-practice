...
export function PreflopTrainer({ entries, setup }: Props) {
  const [hand, setHand] = useState<HandState>(() => startHand(setup))
      ...
      <section className="card">
        ...
        {phase === 'hero' && decision ? (
          ...
          <div className="verdict" aria-live="polite">
            <strong>{hand.ended.message}</strong>
            {decisionsThisHand > 0 ? (
          ...
          <div className="actions actions--waiting" aria-live="polite">
            {acting ? `${POSITION_LABELS[acting.position]} is thinking…` : ''}
          </div>
        )}
      </section>
